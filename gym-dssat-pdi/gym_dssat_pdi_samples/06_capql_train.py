"""
CAPQL — Concave-augmented Pareto Q-Learning for SmartFarm multi-objective control.

Algorithm (TD3 backbone):
  1. Preference conditioning : w ~ Dirichlet([1,1,1]) per episode
  2. Concave augmentation    : r_aug = R_daily + done*(w·R⃗ + λ·mean(log(Rᵢ+1+ε)))
  3. Hindsight pref relabeling: resample w for each transition in minibatch
  4. Twin delayed critic      : reduces Q overestimation (TD3)
  5. Replay buffer            : stores crop_obs (11) and w (3) separately
                                so relabeling can swap w without storing obs twice

Objectives (reward vector R⃗):
  [0] R_yield      — grain yield vs baseline 7620 kg/ha       ∈ [-1,+1]
  [1] R_ane        — N efficiency (yield/N), floor 5000 kg/ha ∈ [-1,+1]
  [2] R_water_eff  — water efficiency (target ≤ 400 mm)       ∈ [-1,+1]

Evaluation corners:
  w=[1,0,0] yield-only   w=[0,1,0] N-efficiency   w=[0,0,1] water   w=[1/3,1/3,1/3] balanced

Produces in /workspace/:
  capql_episode_log.csv
  capql_eval_results.csv
  capql_plot_training.png
  capql_plot_losses.png

Run inside Docker:
    cd /workspace/gym-dssat-pdi/gym_dssat_pdi_samples
    /opt/gym_dssat_pdi/bin/python3 -u 06_capql_train.py
"""
import csv
import os
import sys
...
CAPQLEnv = import_module('capql_env').CAPQLEnv
...
WARMUP_STEPS   = 5_000        # random actions before learning
BUFFER_SIZE    = 200_000
BATCH_SIZE     = 256
LR_ACTOR       = 3e-4
LR_CRITIC      = 3e-4
...
POLICY_FREQ    = 2             # update actor every N critic updates
GRAD_CLIP      = 1.0          # gradient clipping (both nets)

# Exploration noise (in action space units, decays linearly)
EXPL_NOISE_START = 0.3        # fraction of action range
EXPL_NOISE_END   = 0.05
EXPL_NOISE_STEPS = 800_000    # steps to decay over

# Target-policy smoothing noise (TD3, in action space units)
TP_NOISE_STD  = np.array([8.0, 2.0], dtype=np.float32)   # [anfer, amir]
TP_NOISE_CLIP = np.array([20.0, 5.0], dtype=np.float32)

# CAPQL-specific
AUG_LAMBDA    = 0.5            # concave augmentation coefficient λ
AUG_EPS       = 0.1            # ε in log(Rᵢ + 1 + ε) for numerical stability
USE_RELABEL   = True           # hindsight preference relabeling

N_OBJECTIVES  = 3
OBS_DIM       = 14             # 11 crop+SoS + 3 preference w
ACTION_DIM    = 2
HIDDEN        = 256

ACT_LOW   = np.array([0.0,   0.0], dtype=np.float32)
ACT_HIGH  = np.array([200.0, 50.0], dtype=np.float32)
ACT_RANGE = ACT_HIGH - ACT_LOW          # [200, 50]

MODEL_DIR    = '/tmp/capql'
EPISODE_LOG  = '/workspace/capql_episode_log.csv'
EVAL_LOG     = '/workspace/capql_eval_results.csv'
PLOT_TRAIN   = '/workspace/capql_plot_training.png'
PLOT_LOSSES  = '/workspace/capql_plot_losses.png'
...
# Torch tensors for action scaling (used inside Actor)
...
class Actor(nn.Module):
    """
    π_θ(obs) → action ∈ [ACT_LOW, ACT_HIGH]

    obs includes w in its last 3 dims, so the policy is naturally
    conditioned on the current preference vector.
    tanh output ∈ [-1,+1] is linearly mapped to the action range.
    """
...
class TwinCritic(nn.Module):
    """
    Q_φ1(obs, action),  Q_φ2(obs, action)  → scalar × 2  (TD3 twin)

    Input: concat(obs, action) = 14 + 2 = 16 dims.
    """
...
class ReplayBuffer:
    """
    Stores crop_obs (11-dim, WITHOUT w) and w separately so that
    hindsight preference relabeling can swap w at sampling time
    without duplicating observation storage.

    Full obs is reconstructed as concat(crop_obs, w_batch) when sampling.
    """
    ...
    def sample(self, batch_size):
        ...
        # Hindsight relabeling: resample w for each transition
        if USE_RELABEL:
        ...
        # Rebuild full 14-dim obs with relabeled w
...
def augmented_reward(r_daily, reward_vec, done, w):
    ...
    All inputs are (batch, *) torch tensors.
    The concave term λ·mean(log(·)) covers non-convex Pareto regions
    that w·R⃗ alone (linear scalarization) cannot reach.
    """
    scalarized = (w * reward_vec).sum(dim=-1, keepdim=True)          # (B,1)
    concave    = AUG_LAMBDA * torch.mean(
        torch.log(reward_vec + 1.0 + AUG_EPS), dim=-1, keepdim=True  # (B,1)
    )
...
class CAPQLAgent:
    ...
    def update(self, buffer):
        ...
        # ---- Augmented reward ----
        r_aug = augmented_reward(r_daily, reward_vec, done, w)   # (B,1)
...
def train():
    ...
    ep_count  = 0
    ep_daily  = 0.0    # cumulative R_daily for this episode
    ep_length = 0
    ...
    for step in range(1, TOTAL_STEPS + 1):
        ...
        # Store transition (crop obs and w are separated for hindsight relabeling)
        ...
        if done:
            ...
            # True cumulative reward (natural scalarization, no augmentation)
...
def evaluate(agent):
    print("\n" + "=" * 78)
    print("EVAL — CAPQL preference corners")
    print("=" * 78)
    ...
    for corner_name, w_corner in EVAL_CORNERS.items():
        ...
        for ep in range(EVAL_EPISODES):
            ...
            # Fix preference corner for the whole episode
            ...
            while not done:
                ...
                obs[-3:]  = w_corner    # keep w fixed throughout episode
                cum_daily += r_daily
                ep_len    += 1
...
def _plot_training(ep_log):
    ...
    fig.suptitle(
        'CAPQL Training Curves — Concave-augmented Pareto Q-Learning\n'
    ...
    if c_loss:
        ...
        fig2.suptitle('CAPQL Loss Curves', fontsize=12, fontweight='bold')
