...
!     Store plant module data for use in ETPHOT.
     ...
     CALL pdi_transaction_begin("send_state_cotton")
     DAP = MAX(0,TIMDIF(YRPLT,YRDOY))
     CALL pdi_expose("DAP", DAP, pdi_out)
     CALL pdi_expose("GRNWT", SDWT, pdi_out) ! GRNWT changed to SDWT to match output files
     CALL pdi_expose("SWFAC", SWFAC, pdi_out)
     CALL GET("MGMT","TOTIR", TOTIR)
     CALL pdi_expose("TOTIR", TOTIR, pdi_out)
