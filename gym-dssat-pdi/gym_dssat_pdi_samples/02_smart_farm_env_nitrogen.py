"""
SmartFarmSoSEnv_Nitrogen — reward design tuned for NITROGEN MINIMISATION.

Objective: reduce total N application while maintaining acceptable yield.

Key changes vs base reward:
  Daily   — W_RESOURCE 0.15→0.30 (stronger per-step N application penalty)
           — W_FERT    0.30→0.20 (less reward for N uptake/availability)
  Seasonal— N_EXCESS   200→100 kg/ha (penalty fires at 100 kg/ha N)
           — R_penalty split 30% water / 70% N (was 50/50)
           — W_ANE     0.30→0.50 (N efficiency is now dominant objective)
           — W_YIELD   0.40→0.20 (significant yield sacrifice accepted)
           — W_HIAD    0.10→0.05
           — W_PENALTY 0.20→0.25
  Fix     — breaks the N↑yield↑ coupling: extra N now unprofitable
"""
import gym
import numpy as np
...
class SmartFarmSoSEnv:
    ...
    IRRIG_EXCESS = 500.0
    N_EXCESS     = 100.0   # ↓ was 200 — penalty fires at 100 kg/ha N

    DAILY_SCALE = 0.01

    # Daily weights — penalise N application harder, reward N uptake less
    W_WATER    = 0.35
    W_FERT     = 0.20
    W_RESOURCE = 0.30
    W_LOSSES   = 0.15

    # Seasonal weights — yield floor in R_ane blocks the zero-N exploit
    W_YIELD   = 0.25   # yield still matters (floor enforces minimum)
    W_HIAD    = 0.10
    W_ANE     = 0.40   # dominant but not extreme
    W_PENALTY = 0.25
    ...
    def _compute_seasonal_reward(self, crop_obs):
        ...
        # Yield floor: ANE only rewards efficiency if crop actually grew.
        # Without this, agent exploits ANE by applying near-zero N (yield→0,
        # ANE→∞ clips to +1.0). Floor set at 5000 kg/ha (viable crop).
        ...
        if grnwt >= MIN_VIABLE_YIELD:
            ...
            R_ane = -1.0  # crop failure — N efficiency is meaningless
        ...
        # N-min: 70% weight on N penalty, 30% on irrigation penalty
        R_penalty   = 0.30 * r_irrig_pen + 0.70 * r_n_pen
