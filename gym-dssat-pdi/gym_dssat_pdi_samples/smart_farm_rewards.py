...
# v5: K_FERT/K_LEACH/K_DENIT scaled up (5x / 2x / 2.2x) to make daily fert cost
#     dominate the daily uptake bonus once anfer > ~5 kg/day. Without this the
#     per-day fert reward was +0.36 even at fert corner, so the agent was paid
#     to apply N daily and totals stayed at ~1200 kg/ha. With v5 the per-day
#     reward turns negative beyond ~6 kg/day, pushing agent to ~300-500 N total.
K_IRRIG = 0.012       # per mm/day — was 0.004 (too cheap; agent over-irrigated)
K_FERT = 0.040        # v5: per kg N/ha/day (was 0.008; 5x scale)
K_RUNOFF = 0.03       # per mm/day
K_LEACH = 0.050       # v5: per kg N/ha/day leached (was 0.025; 2x scale)
K_DENIT = 0.040       # v5: per kg N/ha/day denitrified (was 0.018; 2.2x scale)

# Season totals — present but not dominant over yield harvest term
# v5: K_SEASON_FERT bumped from 0.006 to 0.025 (4x) so end-of-season lump
#     penalty contributes meaningfully (-30 at 1200 kg N vs -7.2 before).
K_SEASON_WATER = 0.005   # v4: bump from 0.003 to reinforce water-saving signal
K_SEASON_FERT = 0.025    # v5: was 0.006
